...
func (c *Creator) Create(enum *input.Enum) (*output.TemplateInfo, error) {
	...
	for _, element := range enum.Elements {
		if element.SettingAccessorType != input.SettingAccessorType_All && element.SettingAccessorType != input.SettingAccessorType_OnlyClient {
			continue
		}
	...
	return &output.TemplateInfo{
		Data:     buf.Bytes(),
		FilePath: core.JoinPath("gen-proto/client/master", fileName),
	}, nil
}
