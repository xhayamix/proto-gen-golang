...
func (r *{{ .CamelName }}Repository) SelectAll(ctx context.Context) (transaction.{{ .GoName }}Slice, error) {
	...
	slice := make(transaction.{{ .GoName }}Slice, 0)
	for rows.Next() {
		entity := &transaction.{{ .GoName }}{}
...
func (r *{{ .CamelName }}Repository) SelectAllOffset(ctx context.Context, offset, limit int) (transaction.{{ .GoName }}Slice, error) {
...
func (r *{{ .CamelName }}Repository) SelectAllByTx(ctx context.Context, _tx database.ROTx) (transaction.{{ .GoName }}Slice, error) {
...
func (r *{{ .CamelName }}Repository) SelectByPKs(ctx context.Context, pks transaction.{{ .GoName }}PKs) (transaction.{{ .GoName }}Slice, error) {
    var entities transaction.{{ .GoName }}Slice
    for _, pk := range pks {
...
func (r *{{ .CamelName }}Repository) SelectByPK(ctx context.Context{{ range $pk := .PKColumns }}, {{ .GoName }}_ {{ .Type }}{{ end }}) (*transaction.{{ .GoName }}, error) {
...
func (r *{{ .CamelName }}Repository) SelectByTx(ctx context.Context, _tx database.ROTx{{ range $pk := .PKColumns }}, {{ .GoName }}_ {{ .Type }}{{ end }}) (*transaction.{{ .GoName }}, error) {
	tx, err := mysql.ExtractTx(_tx)
	if err != nil {
		return nil, cerrors.Stack(err)
	}
...
func (r *{{ $camelName }}Repository) SelectBy{{ range $j, $col := $cols }}{{ if $j }}And{{ end }}{{ $col.GoName }}{{ end -}}
  (ctx context.Context{{ range $cols }}, {{ .GoName }} {{ .Type }}{{ end }}) (transaction.{{ $name }}Slice, error) {
	...
	slice := make(transaction.{{ $name }}Slice, 0)
	for rows.Next() {
		entity := &transaction.{{ $name }}{}
		ptrs := entity.PtrFromMapping(cols)
		if err := rows.Scan(ptrs...); err != nil {
			return nil, cerrors.Wrap(err, cerrors.Internal)
		}
...
{{ end }}
func (r *{{ .CamelName }}Repository) Insert(ctx context.Context, _tx database.RWTx, entity *transaction.{{ .GoName }}) error {
	tx, err := mysql.ExtractTx(_tx)
	if err != nil {
	...
	vals := entity.PtrFromMapping(transaction.{{ .GoName }}Cols)
...
func (r *{{ .CamelName }}Repository) BulkInsert(ctx context.Context, _tx database.RWTx, entities transaction.{{ .GoName }}Slice, replace bool) error {
	tx, err := mysql.ExtractTx(_tx)
	if err != nil {
		return cerrors.Stack(err)
	}

	columnNames := transaction.{{ .GoName }}Cols
	...
	recordsList := make([]transaction.{{ .GoName }}Slice, 0, int64(math.Ceil(float64(recordCount)/float64(mysql.PlaceholderLimit))))
	tempRecordsCap := recordCount
	if recordCount > mysql.PlaceholderLimit {
		tempRecordsCap = mysql.PlaceholderLimit
	}
	tempRecords := make(transaction.{{ .GoName }}Slice, 0, tempRecordsCap)
	placeholderCount := 0
	...
	for i, record := range entities {
		...
		if placeholderCount > mysql.PlaceholderLimit { // 上限を超えた場合
			...
			tempRecords = make(transaction.{{ .GoName }}Slice, 0, mysql.PlaceholderLimit) // 上限を超えないかも知れないが一応上限までCapを確保して初期化
		}
...
func (r *{{ .CamelName }}Repository) Update(ctx context.Context, _tx database.RWTx, entity *transaction.{{ .GoName }}) error {
	tx, err := mysql.ExtractTx(_tx)
	if err != nil {
		return cerrors.Stack(err)
	}
...
func (r *{{ .CamelName }}Repository) Delete(ctx context.Context, _tx database.RWTx, entity *transaction.{{ .GoName }}) error {
	tx, err := mysql.ExtractTx(_tx)
	if err != nil {
		return cerrors.Stack(err)
	}
...
func (r *{{ .CamelName }}Repository) BulkDelete(ctx context.Context, _tx database.RWTx, entities transaction.{{ .GoName }}Slice) error {
	if len(entities) == 0 {
		return nil
	}
